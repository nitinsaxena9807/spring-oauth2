...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	@Autowired
	@Qualifier("oauth2authFailureHandler")
	ExceptionHandlingSecurityConfiguration exceptionHandlingSecurityConfig;
	...
	protected void configure(HttpSecurity http) throws Exception {		
		...
		http
		.authorizeRequests()
		.antMatchers("/register","/login","/error","/h2-console/**","/mylogin","/verify/**").permitAll()
			.anyRequest().authenticated()
			.and()			
			.formLogin().loginPage("/login").defaultSuccessUrl("/hello", true)		
			.and().csrf().disable().rememberMe().key("myremembermekey")
			.and().formLogin().failureHandler(exceptionHandlingSecurityConfig)
